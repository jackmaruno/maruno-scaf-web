...
export class SegurancaService {
    ...
    public autenticar(login: string, senha: string): Observable<any> {
        const url_token = `${environment.rest_url}/token?client_id=${environment.client_id}&client_secret=${environment.client_secret}&grant_type=password`;
    ...
    public setUsuario(response: any): void { 
        localStorage.setItem(Constantes.ACCESS_TOKEN, `${response['token_type']} ${response['access_token']}`);
        localStorage.setItem(Constantes.USUARIO, JSON.stringify(response['usuario']));
        localStorage.setItem(Constantes.LOGIN, response['usuario']['login']);
    }
    ...
    public limparLocalStorage(): void { 
        ...
        localStorage.removeItem(Constantes.LOGIN);
    }
    ...
    public getToken(): string {
        if(this.token == null){
            this.token = localStorage.getItem(Constantes.ACCESS_TOKEN);
        }
        return this.token;
    }
