...
export function DOM_SERVICE_PROVIDER_FACTORY(
  ...
  return parentContainer || new DomService(componentFactoryResolver, appRef, injector, overlayContainer);
}

export const DOM_SERVICE_PROVIDER = {
  // If there is already an DomService available, use that. Otherwise, provide a new one.
  provide: DomService,
  deps: [[new Optional(), new SkipSelf(), DomService], ComponentFactoryResolver, ApplicationRef, Injector, OverlayContainer],
